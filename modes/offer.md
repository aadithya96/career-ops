...
Table with:
- Detected archetype
- Domain (backend/platform/integration/full-stack/leadership)
- Function (build/lead/consult/deploy)
- Seniority
...
- If Senior Backend → prioritize production APIs, performance, reliability, clean architecture
- If Staff/Principal → prioritize architectural decisions, cross-team influence, technical direction
- If Platform → prioritize CI/CD ownership, observability, DevEx improvements
- If Backend Full Stack → prioritize end-to-end delivery, .NET backend + thin frontend
- If Solutions/Integration → prioritize API design, third-party integrations, enterprise connectivity
- If Engineering Lead → prioritize delivery track record, mentoring, hands-on leadership
...
2. **"Sell senior without lying" plan**: specific phrases adapted to the archetype, concrete achievements to highlight, how to position the candidate's experience as an advantage
...
**Comp scoring rule:** If the JD does not disclose a salary range, score this dimension **3.0 (neutral)** — do NOT penalize. Only score below 3.0 if the JD explicitly states a range below the candidate's target (see `config/profile.yml → compensation.target_ctc`). Most postings don't show budget; absence of data is not a red flag.
...
- Senior Backend → emphasize production reliability, performance wins, clean architecture decisions
- Staff/Principal → emphasize architectural trade-offs, cross-team influence, technical direction
- Platform → emphasize developer velocity improvements, CI/CD ownership, observability
- Solutions/Integration → emphasize clean API design, integration complexity, enterprise delivery
- Engineering Lead → emphasize delivery wins, team growth, hands-on mentoring
